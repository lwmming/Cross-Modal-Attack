...
class VideoClsDataset(Dataset):
    ...
    def loadvideo_decord(self, sample, clip_ind, sample_rate_scale=1):
        ...
        converted_len = int(self.clip_len * self.frame_sample_rate) # 32x2
        seg_len = len(vr) // self.num_segment   # 250/1
        # import ipdb;ipdb.set_trace()
        all_index = []
        for i in range(self.num_segment):
            if seg_len <= converted_len:
                ...
                if clip_ind == -1:
                    end_idx = seg_len - 1   # 250-1=249
                else:
                    np.random.seed(clip_ind)
                    end_idx = np.random.randint(converted_len, seg_len)
                str_idx = end_idx - converted_len   # 249-64
                index = np.linspace(str_idx, end_idx, num=self.clip_len)  # (249-64):249, shape:32
                index = np.clip(index, str_idx, end_idx - 1).astype(np.int64)
            index = index + i*seg_len   # [185, 187, 189, ..., 248]
