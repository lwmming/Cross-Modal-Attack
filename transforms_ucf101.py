...
class attack_ucf101(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.temporal_transform is not None:
            frame_indices = self.temporal_transform(frame_indices)  # 变为32帧
        ...
        # print(clip[0].shape)  C*H*W
        clip = torch.stack(clip, 0).permute(1, 0, 2, 3)  # C*N*H*W
        #print(clip.shape)  # 3*32*224*224
        return clip, target
    ...
    def _make_dataset(self, setting):
        ...
        for idx, row in df.iterrows():
            clip_path = os.path.join('./UCF101-Examples/', row['path'].split('/')[-1])
